...
#include <string.h>
#include <sha1.h>
//在unsigned char 数组中得到无符号长整数( unsigned long * ) buffer[s]
//16位存储, 1个char字节占2个字节位置
unsigned long FixChar16Bit_Get(unsigned char * pBuf, unsigned char offset)
{
	unsigned long temp = 0;
	unsigned char i;

	offset <<= 2;//unsigned long 为4字节, 如果传offset=1,实际为4的地址
	pBuf += offset;
	for(i = 0; i < 4; i++)
	{
		temp <<= 8;
...
void FixChar16Bit_Put(unsigned char * pBuf, unsigned char offset, unsigned long value)
{
	unsigned char i;
...
void SHA1Transform( unsigned long * state /*[5]*/, unsigned char * buffer /*[64]*/ )  {
 	...
 	*/
 	...
 	for ( t = 0; t < 80; t ++ )
 	{
 		...
 		*/
 			FixChar16Bit_Put(buffer, s, _lrol_(
 					FixChar16Bit_Get(buffer, (s+13) & 15) ^
 					FixChar16Bit_Get(buffer, (s+8) & 15) ^
 					FixChar16Bit_Get(buffer, (s+2) & 15) ^
 					FixChar16Bit_Get(buffer, s),1
 							));
 		}
 		...
 		*/
 		...
 		temp += (_lrol_( a, 5 ) + e + FixChar16Bit_Get(buffer, s) + kt);
 		e = d;
 		d = c;
 		c = _lrol_( b, 30 );
 		b = a;
